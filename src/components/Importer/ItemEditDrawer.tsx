  ...
  const handleSave = () => {
    ...
    const verifiedChecks = [
      ...
      },
      {
        ...
        passed: true,
        formula: `健保(¥${healthInsurance.toLocaleString()}) + 厚年(¥${welfarePension.toLocaleString()}) + 雇保(¥${employmentInsurance.toLocaleString()}) = 社保計(¥${salarySocTotal.toLocaleString()})`,
      },
      {
        name: '控除合計與差引手取平衡 (實手取確認)',
        passed: true,
        formula: `總支給(¥${salaryTotalGross.toLocaleString()}) - 總控除計(¥${salaryTotalDeductions.toLocaleString()}) = 差引支給額(¥${salaryNetPay.toLocaleString()})`,
      },
      {
        name: '自適應學習庫特徵同步',
        passed: true,
